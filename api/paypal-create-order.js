...
module.exports = async function handler(req, res) {
  ...
  const bodySize = ensureJsonBodySize(res, req.body, 4 * 1024, 'PayPal order request');
  if (bodySize.handled) return;
  const rateLimit = await enforceRateLimit(req, res, {
    key: 'paypal-create-order', limit: 20, windowMs: 10 * 60 * 1000
  });
  ...
  try {
    ...
    const orderResponse = await fetch(`${getApiBaseUrl()}/v2/checkout/orders`, {
      ...
      headers: {
        ...
        'PayPal-Request-Id': createOrderRequestId()
      },
      body: JSON.stringify({
        intent: 'CAPTURE',
        purchase_units: [{
