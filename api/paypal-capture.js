...
const { getAccessToken, getApiBaseUrl, getEnvironment } = require('../lib/paypal');
...
module.exports = async function handler(req, res) {
  ...
  const bodySize = ensureJsonBodySize(res, req.body, 4 * 1024, 'PayPal capture request');
  if (bodySize.handled) return;
  const rateLimit = await enforceRateLimit(req, res, {
    key: 'paypal-capture', limit: 30, windowMs: 10 * 60 * 1000
  });
