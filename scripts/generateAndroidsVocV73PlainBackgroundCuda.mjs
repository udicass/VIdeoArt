...
const outputDir = 'D:\\SD_Deforum_Fresh\\outputs\\androids-text-frames\\2026-08-11\\androids_dream_VOC_V6_PLAINBACKGROUND_CUDA_V73_25SEC';
const cleanMovie = path.join(previewRoot, 'androids_dream_VOC_V6_PLAINBACKGROUND_CUDA_V73_25SEC_1080.mp4');
const output = path.join(previewRoot, 'androids_dream_VOC_V6_PLAINBACKGROUND_CUDA_V73_STROBE_25SEC_1080.mp4');
const contact = path.join(previewRoot, 'androids_dream_VOC_V6_PLAINBACKGROUND_CUDA_V73_STROBE_25SEC_contact.jpg');
const temp = path.join(previewRoot, '_voc_v6_plainbackground_v73_temp.mp4');
...
if (schedule.length !== 25 || timeline.beats.length < 25) throw new Error('V73 requires 25 V47 beats and source assignments');
...
for (let index = 0; index < schedule.length; index += 1) {
  ...
  process.stdout.write(`generated V73 plain-background frame ${index + 1}/25 from identity ${identity}\n`);
}
