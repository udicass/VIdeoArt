...
const outputDir = 'D:\\SD_Deforum_Fresh\\outputs\\androids-text-frames\\2026-08-14\\androids_dream_VOC_V6_MALE_TO_FEMALE_FACES_V99';
const contact = path.join(previewRoot, 'androids_dream_VOC_V6_MALE_TO_FEMALE_FACES_V99_contact.jpg');
...
for (let index = 0; index < 12; index += 1) {
  ...
  const response = await fetch('http://127.0.0.1:7860/sdapi/v1/img2img', {
    ...
    body: JSON.stringify({
      ...
      prompt: [prompt, 'female version with clean eyes and stable facial anatomy'].join(', '),
      ...
      scheduler: 'Karras',
      restore_faces: true,
      seed: 719201000 + index,
      batch_size: 1,
      n_iter: 1,
      save_images: false
    })
  });
  if (!response.ok) throw new Error(`Female face ${index + 1}: ${response.status} ${await response.text()}`);
  const json = await response.json();
  const encoded = String(json.images?.[0] || '').replace(/^data:image\/\w+;base64,/, '');
  if (!encoded) throw new Error(`Female face ${index + 1} returned no image`);
  writeFileSync(outPath, Buffer.from(encoded, 'base64'));
  process.stdout.write(`generated V99 female face ${index + 1}/12 from male face ${index + 1}\n`);
}
