...
const contact = path.join(previewRoot, 'androids_dream_VOC_V6_MALE_TO_FEMALE_BLACKBG_FACES_V105_contact.jpg');
...
const prompt = [
  ...
  'deep pure black background, empty black negative space, no scenery no objects',
  'bright clearly lit face with natural skin, high contrast cinematic lighting, face clearly separated from the black background',
  ...
  'sharp coherent facial anatomy, direct camera gaze'
].join(', ');
const negativePrompt = [
  ...
  'gray background, light background, bright background, washed out, low contrast, flat lighting, fog, haze',
...
for (let index = 0; index < 12; index += 1) {
  ...
  const response = await fetch('http://127.0.0.1:7860/sdapi/v1/img2img', {
    ...
    body: JSON.stringify({
      ...
      prompt: [prompt, 'bright face on deep black background, clearly female'].join(', '),
      ...
      scheduler: 'Karras',
      restore_faces: true,
      seed: 720001000 + index,
      batch_size: 1,
      n_iter: 1,
      save_images: false
    })
  });
  if (!response.ok) throw new Error(`Black-bg female face ${index + 1}: ${response.status} ${await response.text()}`);
  const json = await response.json();
  const encoded = String(json.images?.[0] || '').replace(/^data:image\/\w+;base64,/, '');
  if (!encoded) throw new Error(`Black-bg female face ${index + 1} returned no image`);
  writeFileSync(outPath, Buffer.from(encoded, 'base64'));
  process.stdout.write(`generated V105 black-bg female face ${index + 1}/12 from male face ${index + 1}\n`);
}
...
console.log(JSON.stringify({ generatedFaces: 12, sourceSet: 'V97-male', outputDir, contact, denoisingStrength: 0.72, restoreFaces: true }, null, 2));
