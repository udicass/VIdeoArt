...
const framesDir = 'D:\\SD_Deforum_Fresh\\outputs\\androids-text-frames\\2026-08-14\\androids_dream_VOC_ANDROGYNOUS_TWO_CHARS_CUDA_V94_20SEC';
const temp = path.join(previewRoot, '_voc_v94_androgynous_two_chars_temp.mp4');
const output = path.join(previewRoot, 'androids_dream_VOC_ANDROGYNOUS_TWO_CHARS_V94_TEST_20SEC_1080.mp4');
const contact = path.join(previewRoot, 'androids_dream_VOC_ANDROGYNOUS_TWO_CHARS_V94_TEST_20SEC_contact.jpg');

for (let index = 1; index <= 20; index += 1) {
  const framePath = path.join(framesDir, `andro_face_${String(index).padStart(4, '0')}.png`);
  if (!existsSync(framePath)) throw new Error(`Missing V94 frame: ${framePath}`);
}
