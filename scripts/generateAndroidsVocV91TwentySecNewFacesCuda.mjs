...
const sourceDir = 'D:\\SD_Deforum_Fresh\\outputs\\androids-text-frames\\2026-08-12\\androids_dream_VOC_V6_TWENTY_CLEAN_NEW_FACES_CUDA_V89';
const outputDir = 'D:\\SD_Deforum_Fresh\\outputs\\androids-text-frames\\2026-08-14\\androids_dream_VOC_V6_TWENTY_NEW_FACES_CUDA_V91_20SEC';
const contact = path.join(previewRoot, 'androids_dream_VOC_V6_TWENTY_NEW_FACES_CUDA_V91_20SEC_contact.jpg');
const sourceFor = Array.from({ length: 20 }, (_, index) => (index % 20) + 1);
const variations = [
...
function sourcePath(face) {
  return path.join(sourceDir, `new_face_${String(face).padStart(4, '0')}.png`);
}
function outputPath(index) {
  return path.join(outputDir, `voc_face_${String(index + 1).padStart(4, '0')}.png`);
}

for (const face of sourceFor) {
  if (!existsSync(sourcePath(face))) throw new Error(`Missing V89 source face: ${sourcePath(face)}`);
}
...
for (let index = 0; index < 20; index += 1) {
  ...
  const response = await fetch('http://127.0.0.1:7860/sdapi/v1/img2img', {
    ...
    body: JSON.stringify({
      ...
      scheduler: 'Karras',
      restore_faces: true,
      seed: 714601000 + index,
      batch_size: 1,
      n_iter: 1,
      save_images: false
    })
  });
  if (!response.ok) throw new Error(`VOC face ${index + 1}: ${response.status} ${await response.text()}`);
  const json = await response.json();
  const encoded = String(json.images?.[0] || '').replace(/^data:image\/\w+;base64,/, '');
  if (!encoded) throw new Error(`VOC face ${index + 1} returned no image`);
  writeFileSync(outPath, Buffer.from(encoded, 'base64'));
  process.stdout.write(`generated V91 VOC face ${index + 1}/20 from V89 face ${sourceFor[index]}\n`);
}

execFileSync('ffmpeg', [
  '-y', '-loglevel', 'error', '-framerate', '1', '-i', path.join(outputDir, 'voc_face_%04d.png'),
  '-vf', 'scale=256:256:flags=lanczos,tile=5x4', '-frames:v', '1', '-update', '1', contact
], { stdio: 'inherit' });

console.log(JSON.stringify({ generatedFaces: 20, sourceFaces: 20, sourceSet: 'V89', outputDir, contact, denoisingStrength: 0.40, restoreFaces: true }, null, 2));
