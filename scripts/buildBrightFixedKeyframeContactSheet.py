...
source = Path("outputs/deforum-merged-previews/CARA_keyframes_bright_fixed")
out_path = Path("outputs/deforum-merged-previews/CARA_keyframes_bright_fixed_labeled_contact.jpg")

cols, rows = 4, 6
tile_w, tile_h = 320, 568
sheet = np.zeros((rows * tile_h, cols * tile_w, 3), dtype=np.uint8)

for idx in range(1, 25):
    image = cv2.imread(str(source / f"keyframe_{idx:04d}.png"))
    if image is None:
        continue
