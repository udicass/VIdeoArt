...
const facesDir = 'D:\\SD_Deforum_Fresh\\outputs\\androids-text-frames\\2026-08-12\\androids_dream_VOC_V6_TWENTY_CLEAN_NEW_FACES_CUDA_V89';
...
const facesTemp = path.join(previewRoot, '_voc_v6_v89_clean_faces_temp.mp4');
const facesMovie = path.join(previewRoot, 'androids_dream_VOC_V6_TWENTY_CLEAN_NEW_FACES_CUDA_V90_30SEC_1080.mp4');
const output = path.join(previewRoot, 'androids_dream_VOC_V16_V36_V74_V89_CLEAN20FACES_V90_COMPLETE_60SEC_1080.mp4');
const contact = path.join(previewRoot, 'androids_dream_VOC_V16_V36_V74_V89_CLEAN20FACES_V90_COMPLETE_contact.jpg');
const facesContact = path.join(previewRoot, 'androids_dream_VOC_V6_TWENTY_CLEAN_NEW_FACES_CUDA_V90_30SEC_contact.jpg');
...
for (let index = 1; index <= 20; index += 1) {
  ...
  if (!existsSync(facePath)) throw new Error(`Missing V89 face: ${facePath}`);
}
...
console.log(JSON.stringify({ existingSeconds: 30, cleanNewFaces: 20, cleanNewFacesSeconds: 30, totalSeconds: 60, output, contact, facesContact }, null, 2));